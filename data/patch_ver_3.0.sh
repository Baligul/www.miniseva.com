...
# Run the patch to update the existing latest DB to support question types
# ------------------------------------------------------------------------------
echo "=== Updating the latest RS db using patch ==="
#$SUDO -u $ADMIN_ROLE $PSQL -d $database < ./pg/update/patch_ver_3.0.sql
$PSQL -h localhost -p 5432 -U rs -w -d $database  < ./pg/update/patch_ver_3.0.sql > restore-db.log 2>&1 && grep -i error restore-db.log